...
<main class="container mx-auto py-4">
	...
	<form id="form" on:submit|preventDefault={updateProfile} class="grid gap-4 px-4 md:grid-cols-2">
		...
		<button
			type="submit"
			disabled={loading}
			class="relative mt-4 flex w-fit items-center justify-center bg-black px-4 py-2 text-white"
		>
			{#if loading}
				<img class="absolute left-16 h-8" src={loadingSpinner} alt="loading" />
			{/if}
